  ...
  if (JSnum == 1) {
      ...
      if (mask[1] & DECODE_green){
        if(mask[1] & cmp_packet[1]){
          ...
          Serial.println(F(" JS1: Green Released "));}}
      if (mask[1] & DECODE_l2){
        if(mask[1] & cmp_packet[1]){
          Keyboard.press(JS1_l2);
          Serial.println(F(" JS1: L2 Pressed "));}
        else{
          Keyboard.release(JS1_l2);
          Serial.println(F(" JS1: L2 Released "));}}
      if (mask[1] & DECODE_blue){
        if(mask[1] & cmp_packet[1]){
          Keyboard.press(JS1_blue);
          Serial.println(F(" JS1: Blue Pressed "));}
        else{
          Keyboard.release(JS1_blue);
          Serial.println(F(" JS1: Blue Released "));}}
      if (mask[1] & DECODE_r2){
        if(mask[1] & cmp_packet[1]){
          Keyboard.press(JS1_r2);
          Serial.println(F(" JS1: R2 Pressed "));}
        else{
          Keyboard.release(JS1_r2);
          Serial.println(F(" JS1: R2 Released "));}}
      if (mask[1] & DECODE_start){
        if(mask[1] & cmp_packet[1]){
          Keyboard.press(JS1_start);
          Serial.println(F(" JS1: Start Pressed "));}
        else{
          Keyboard.release(JS1_start);
          Serial.println(F(" JS1: Start Released "));}}
      if (mask[1] & DECODE_select){
        if(mask[1] & cmp_packet[1]){
         Keyboard.press(JS1_select);
         Serial.println(F(" JS1: Select Pressed "));}
        else{
          Keyboard.release(JS1_select);
          Serial.println(F(" JS1: Select Released "));}}
    if((cmp_packet[0]|cmp_packet[1])==0){  // if no buttons are pressed
      ...
      if (mask[0] & DECODE_left){     //if the 'left' d-pad state has changed
        if(mask[0] & cmp_packet[0]){  //if the bit in our packet is '1' 
          ...
          Serial.println(F(" JS2: Left Pressed "));}
        else{                         //otherwise...
          Keyboard.release(JS2_left);
          Serial.println(F(" JS2: Left Released "));}}
      if (mask[0] & DECODE_right){
        if(mask[0] & cmp_packet[0]){
          Keyboard.press(JS2_right);
          Serial.println(F(" JS2: Right Pressed "));}
        else{
          Keyboard.release(JS2_right);
          Serial.println(F(" JS2: Right Released "));}}
      if (mask[0] & DECODE_down){  
        if(mask[0] & cmp_packet[0]){
          Keyboard.press(JS2_down);
          Serial.println(F(" JS2: Down Pressed "));}
        else{
          Keyboard.release(JS2_down);
          Serial.println(F(" JS2: Down Released "));}}
      if (mask[0] & DECODE_up){
        if(mask[0] & cmp_packet[0]){
          Keyboard.press(JS2_up);
          Serial.println(F(" JS2: Up Pressed "));}
        else{
          Keyboard.release(JS2_up);
          Serial.println(F(" JS2: Up Released "));}}
      if (mask[0] & DECODE_r1){
        if(mask[0] & cmp_packet[0]){
          Keyboard.press(JS2_r1);
          Serial.println(F(" JS2: R1 Pressed "));}
        else{
          Keyboard.release(JS2_r1);
          Serial.println(F(" JS2: R1 Released "));}}
      if (mask[0] & DECODE_l1){
        if(mask[0] & cmp_packet[0]){
          Keyboard.press(JS2_l1);
          Serial.println(F(" JS2: L1 Pressed "));}
        else{
          Keyboard.release(JS2_l1);
          Serial.println(F(" JS2: L1 Released "));}}
      if (mask[0] & DECODE_red){
        if(mask[0] & cmp_packet[0]){
          Keyboard.press(JS2_red);
          Serial.println(F(" JS2: Red Pressed "));}
        else{
          Keyboard.release(JS2_red);
          Serial.println(F(" JS2: Red Released "));}}
      if (mask[0] & DECODE_yellow){
        if(mask[0] & cmp_packet[0]){
          Keyboard.press(JS2_yellow);
          Serial.println(F(" JS2: Yellow Pressed "));}
        else{
          Keyboard.release(JS2_yellow);
          Serial.println(F(" JS2: Yellow Released "));}}
      if (mask[1] & DECODE_green){
        if(mask[1] & cmp_packet[1]){
          Keyboard.press(JS2_green);
          Serial.println(F(" JS2: Green Pressed "));}
        else{
          Keyboard.release(JS2_green);
          Serial.println(F(" JS2: Green Released "));}}
      if (mask[1] & DECODE_l2){
        if(mask[1] & cmp_packet[1]){
          Keyboard.press(JS2_l2);
          Serial.println(F(" JS2: L2 Pressed "));}
        else{
          Keyboard.release(JS2_l2);
          Serial.println(F(" JS2: L2 Released "));}}
      if (mask[1] & DECODE_blue){
        if(mask[1] & cmp_packet[1]){
          Keyboard.press(JS2_blue);
          Serial.println(F(" JS2: Blue Pressed "));}
        else{
          Keyboard.release(JS2_blue);
          Serial.println(F(" JS2: Blue Released "));}}
      if (mask[1] & DECODE_r2){
        if(mask[1] & cmp_packet[1]){
          Keyboard.press(JS2_r2);
          Serial.println(F(" JS2: R2 Pressed "));}
        else{
          Keyboard.release(JS2_r2);
          Serial.println(F(" JS2: R2 Released "));}}
      if (mask[1] & DECODE_start){
        if(mask[1] & cmp_packet[1]){
          Keyboard.press(JS2_start);
          Serial.println(F(" JS2: Start Pressed "));}
        else{
          Keyboard.release(JS2_start);
          Serial.println(F(" JS2: Start Released "));}}
      if (mask[1] & DECODE_select){
        if(mask[1] & cmp_packet[1]){
          Keyboard.press(JS2_select);
          Serial.println(F(" JS2: Select Pressed "));}
        else{
          Keyboard.release(JS2_select);
          Serial.println(F(" JS2: Select Released "));}}
